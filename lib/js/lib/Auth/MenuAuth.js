 ...
 * @return bool
 */
function hasHashAuth(userMenuAuths, hash, checkUrl) {
  if (allowedUrls.includes(checkUrl)) {
    return true;
  }

  const hasUrl = userMenuAuths.some(menuAuth => {
...
export default function (userMenuAuths, method, checkUrl) {
  return hasHashAuth(userMenuAuths, null, checkUrl);
}
