...
exports.default = function (userAuth, method, checkUrl) {
...
function hasHashAuth(userAuth, hash, checkUrl) {
    ...
    var hasUrl = userAuth.menuAuths.some(function (menuAuth) {
        return isAuthUrl(checkUrl, menuAuth.menu_url) && isAuthCorrect(hash, menuAuth.auth ? menuAuth.auth : []);
    });
    if (hasUrl) {
        return true;
    }
    return userAuth.menuAuths.filter(function (menuAuth) {
